...
public class MigrationService {
   ...
   public List<OldProperty> searchOldProperty(org.egov.pt.web.contracts.RequestInfoWrapper requestInfoWrapper, OldPropertyCriteria propertyCriteria){
    ...
    public List<Property> updateMigratedProperty(RequestInfoWrapper requestInfoWrapper, OldPropertyCriteria propertyCriteria) {
        ...
        for(OldProperty oldProperty : oldProperties){
            int size = oldProperty.getPropertyDetails().size();
            propertyType = migratePropertyType(oldProperty.getPropertyDetails().get(size-1));
            if(oldProperty.getPropertyId()!=null){
                propertytypeMap.put(oldProperty.getPropertyId(),propertyType);
                propertyIds.add(oldProperty.getPropertyId());
            }

        }
        System.out.println("propertyIds-->"+propertyIds);
        PropertyCriteria propertyCriteria1 = PropertyCriteria.builder().propertyIds(propertyIds).tenantId(propertyCriteria.getTenantId()).build();
        List<Property> properties = propertyService.searchProperty(propertyCriteria1,requestInfoWrapper.getRequestInfo());
        for(int j=0;j<properties.size();j++){
            String type = propertytypeMap.get(properties.get(j).getPropertyId());
            properties.get(j).setPropertyType(type);
            PropertyRequest request = PropertyRequest.builder().requestInfo(requestInfoWrapper.getRequestInfo()).property(properties.get(j)).build();
